...
public:
    ...
    // W poniższych sześciu funkcjach (od get_stem_id() do get_children_end())
    // nie wprowadzamy żadnych zmian, więc wszystkie mają silną odporność na wyjątki.
    ...
    void create(const typename Virus::id_type &id, const std::vector<typename Virus::id_type> &parent_ids) {
        ...
        try {
            // Jeżeli poniższy kod rzuci wyjątek, to musimy cofnąć zmiany, które zostały w nim wprowadzone.
            ...
            for (size_t next_id = 0; next_id < parent_ids.size(); next_id++) {
                if (!insert_results[next_id].second)
                    break;
                // erase() na secie jest noexcept, gdy parametrem jest iterator
                (parent_its[next_id]->second).children.erase(insert_results[next_id].first);
            }
            throw;
        }
    }
...
private:
    ...
    // Funkcje pomocnicze remove.

    void find_nodes_to_remove(
        const Virus::id_type &id,
        std::map<typename Virus::id_type, std::vector<typename Virus_ids_t::iterator>> &parent_ids_to_remove,
        std::vector<typename Nodes_map_t::iterator> &nodes_to_remove
    ) {
    ...
    void find_parents_of_node_to_remove(
        const Virus::id_type &id,
        std::vector<typename Nodes_map_t::iterator> &parent_its,
        std::vector<typename Virus_ptrs_t::iterator> &position_in_children
    ) {
    ...
    void find_affected_nodes(
        const std::map<typename Virus::id_type, std::vector<typename Virus_ids_t::iterator>> &parent_ids_to_remove,
        std::vector<typename Nodes_map_t::iterator> &affected_nodes,
        std::vector<std::vector<typename Virus_ids_t::iterator>> &deleted_parents
    ) {
    ...
    void delete_parents_of_affected_nodes (
        const std::vector<typename Nodes_map_t::iterator> &affected_nodes,
    ...
    void update_parents_of_node_to_remove (
        const std::vector<typename Nodes_map_t::iterator> &parent_its,
    ...
    void delete_nodes(const std::vector<typename Nodes_map_t::iterator> &nodes_to_remove) noexcept {
