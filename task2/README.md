...
Moduł maptel powinien składać się z pliku nagłówkowego [maptel.h](https://github.com/patjed41/JNP1/blob/master/task2/maptel.h) deklarującego
jego interfejs oraz pliku [maptel.cc](https://github.com/patjed41/JNP1/blob/master/task2/maptel.cc) zawierającego jego implementację.
...
- // Wstawia do słownika o identyfikatorze `id` informację o zmianie numeru `tel_src` na numer `tel_dst`. Nadpisuje ewentualną istniejącą informację.

  `void maptel_insert(unsigned long id, char const *tel_src, char const *tel_dst); `

- // Jeśli w słowniku o identyfikatorze `id` jest informacja o zmianie numeru `tel_src`, to ją usuwa. W przeciwnym przypadku nic nie robi.

  `void maptel_erase(unsigned long id, char const *tel_src);`

- // Sprawdza, czy w słowniku o identyfikatorze `id` jest zapisana zmiana numeru `tel_src`. Podąża ciągiem kolejnych zmian. Zapisuje zmieniony numer w `tel_dst`. Jeśli nie ma zmiany numeru lub zmiany tworzą cykl, to zapisuje w `tel_dst` numer `tel_src`. Wartość `len` to rozmiar przydzielonej pamięci wskazywanej przez `tel_dst`.

  `void maptel_transform(unsigned long id, char const *tel_src, char *tel_dst, size_t len);`

Nagłówek modułu maptel powinien udostępniać stałą `TEL_NUM_MAX_LEN` typu size_t o wartości 22.

Przykład użycia znajduje się w pliku [maptel_test1.c](https://github.com/patjed41/JNP1/blob/master/task2/maptel_test1.c).

Ludzie bywają przewrotni i mając kod, który można używać w języku C, chcą go też
używać w C++. Należy zapewnić możliwość użycia pliku nagłówkowego [maptel.h](https://github.com/patjed41/JNP1/blob/master/task2/maptel.h)
...
Przykład użycia znajduje się w pliku [maptel_test2.cc](https://github.com/patjed41/JNP1/blob/master/task2/maptel_test2.cc).
...
[maptel_test1.err](https://github.com/patjed41/JNP1/blob/master/task2/maptel_test1.err) i [maptel_test2.err](https://github.com/patjed41/JNP1/blob/master/task2/maptel_test2.err).
...
Pliki z rozwiązaniem [maptel.h](https://github.com/patjed41/JNP1/blob/master/task2/maptel.h) i [maptel.cc](https://github.com/patjed41/JNP1/blob/master/task2/maptel.cc) należy umieścić w repozytorium
w katalogu
